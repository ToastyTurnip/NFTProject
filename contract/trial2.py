...
def wrap_metadata(lot_id, max_rows, max_columns):
    return sp.map(
        l={
            "max_rows": sp.as_nat(max_rows), 
            "max_columns": sp.as_nat(max_columns),
        }
    )


# create contract of class Nft --- proj start
...
def project():
    ...
    def __init__(self, admin, metadata, ledger, token_metadata):
            # lot_id is right sequence of grid (0,0) has lot_id = 1
            # define lot_id, owner_title, description, image_url, owned, # token-required, max_rows, max_columns
            ...
            self.data = {'max_rows':metadata['max_rows'],
                        'max_columns':metadata['max_columns']
                        }

        @sp.entrypoint
        def mint(self, owner):
            # todo
            assert self.data.lot_id < ((self.data.max_columns * self.data.max_rows) - 1) , "Over the lot count limit"
            ...
            self.data.token_metadata[token_id] = sp.record(
                token_id=token_id, token_info=sp.big_map({
                    "name": "LotNFT",
                    "symbol": bytes_of_string("LTNFT"),
                    "lot_id": self.data.lot_id,
                    "x_coord": sp.nat(x_coord),
                    "y_coord": sp.nat(y_coord),
                    "description": "desc here",
                    "image_url": sp.utils.bytes_of_string("ipfs://sana"),
                    "owner_title": sp.utils.bytes_of_string("ipfs://sana"),
                })
            )
    ...
    class LotFactory(sp.Contract):
        ...
        def create_lot(self, lot_id, owner_title, description, image_url, owned, metadata):
            assert sp.sender == self.data.admin, "User is not an admin"
            
            lotContract = AdLotNFT(self.data.admin, wrap_metadata(lot_id, self.data.max_rows, self.data.max_columns), {})
            self.data.lotContracts[self.data.lotContractCount] = lotContract
